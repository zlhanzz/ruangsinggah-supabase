...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes("Kosongan (Tanpa Perabot)") && lines[i].includes(".map(fac")) {
...
if (foundRtIdx !== -1) {
    ...
    const indentation = lines[foundRtIdx].match(/^\s*/)[0];
    ...
    const newRtBlock = [
        ...
        `${subInd}return (`,
        `${subSubInd}<div className="flex bg-gray-100 p-1 rounded-xl gap-1 mb-3 border border-gray-200/80">`,
        `${subSubSubInd}<button`,
    ...
    // the .map(...) was line foundRtIdx, down to lines[foundRtIdx+15] (which was the checkbox and labels wrapper).
    // Let's replace the original 16 lines (from foundRtIdx to foundRtIdx+15).
    console.log("Replacing Rt lines from", foundRtIdx, "to", foundRtIdx+15);
    lines.splice(foundRtIdx, 16, ...newRtBlock);
} else {
    console.error("Could not find rt block!");
}

// Adjust foundTempIdx after the line insertion in rt block
const newCode = lines.join('\n');
const lines2 = newCode.split(/\r?\n/);

foundTempIdx = -1;
for (let i = 0; i < lines2.length; i++) {
    if (lines2[i].includes("Kosongan (Tanpa Perabot)") && lines2[i].includes(".map(fac")) {
        let isTemp = false;
        for (let j = 1; j <= 5; j++) {
            if (lines2[i+j] && lines2[i+j].includes("temporaryRoom.roomFacilities")) {
                isTemp = true;
            }
        }
        if (isTemp) {
...
if (foundTempIdx !== -1) {
    console.log("Found temp block at line index:", foundTempIdx);
    
    const indentation = lines2[foundTempIdx].match(/^\s*/)[0];
    const subInd = indentation + "    ";
    const subSubInd = subInd + "    ";
    const subSubSubInd = subSubInd + "    ";
    ...
    // Replace the original 16 lines (from foundTempIdx to foundTempIdx+15).
    console.log("Replacing Temp lines from", foundTempIdx, "to", foundTempIdx+15);
    lines2.splice(foundTempIdx, 16, ...newTempBlock);
    
    fs.writeFileSync(targetFile, lines2.join('\n'), 'utf8');
    console.log("Successfully replaced Target Temp!");
} else {
    console.error("Could not find temp block!");
}
