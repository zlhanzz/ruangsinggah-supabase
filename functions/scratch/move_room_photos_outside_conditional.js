...
// 1. Move temporaryRoom Foto Kamar out of conditional block
...
if (tempPhotoStart !== -1 && tempPhotoEnd !== -1) {
  console.log(`Found temporaryRoom photo block at lines ${tempPhotoStart+1} to ${tempPhotoEnd+1}`);
  const photoLines = lines.slice(tempPhotoStart, tempPhotoEnd + 1);
  lines.splice(tempPhotoStart, tempPhotoEnd - tempPhotoStart + 1);
  
  // Find where temporaryRoom facilities block ends to insert photos right under it
  let facEndIdx = -1;
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('Tambah fasilitas kustom...') && i < 3700) {
  ...
  if (facEndIdx !== -1) {
    console.log(`Inserting temporaryRoom photo block after line ${facEndIdx+1}`);
    lines.splice(facEndIdx + 1, 0, ...photoLines);
  }
}

// 2. Move activeRoomIdx Foto Kamar out of conditional block
...
if (activePhotoStart !== -1 && activePhotoEnd !== -1) {
  console.log(`Found activeRoomIdx photo block at lines ${activePhotoStart+1} to ${activePhotoEnd+1}`);
  ...
  // Find where activeRoomIdx facilities block ends to insert photos right under it
  let facEndIdx = -1;
  for (let i = 0; i < lines2.length; i++) {
    if (lines2[i].includes('Tambah fasilitas kustom...') && i > 4000) {
      // Find the closing div of this section
      for (let j = i; j < i + 10; j++) {
  ...
  if (facEndIdx !== -1) {
    console.log(`Inserting activeRoomIdx photo block after line ${facEndIdx+1}`);
    lines2.splice(facEndIdx + 1, 0, ...photoLines);
  }
}
