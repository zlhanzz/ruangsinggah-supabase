...
if (newSilakanIdx !== -1) {
  // Replace the 6 lines starting from newSilakanIdx - 3
...
for (let i = 0; i < lines2.length; i++) {
  if (lines2[i].includes('Status Kamar') && lines2[i].includes('h3')) {
    occurrence++;
    if (occurrence === 2) {
...
if (activeStatusStart !== -1) {
  for (let i = activeStatusStart; i < lines2.length; i++) {
    if (lines2[i] && lines2[i].includes('Skema Tarif / Harga Kamar Section')) {
      activeDetailEnd = i - 1;
      break;
    }
  }
...
if (activeStatusStart !== -1 && activeDetailEnd !== -1) {
  ...
  if (newSelesaiIdx !== -1) {
    console.log("Inserting activeRoomIdx closing wrapper before 'Simpan Perubahan Button' at line", newSelesaiIdx + 1);
