...
if (code.includes(saveSuccessTarget)) {
    ...
    // try to match the other save success message structure
    const saveSuccessTarget2 = cleanString(`            alert('Listing properti & kamar berhasil disimpan! Status pengajuan kini PENDING ONBOARDING.');
            setIsEditingKostManager(null);`);
    const saveSuccessReplacement2 = cleanString(`            alert('Listing properti & kamar berhasil disimpan! Status pengajuan kini PENDING ONBOARDING.');
            closeKostManagerListing();`);
    if (code.includes(saveSuccessTarget2)) {
        code = code.replace(saveSuccessTarget2, saveSuccessReplacement2);
        console.log("4. saveSuccessTarget2 updated to closeKostManagerListing().");
    } else {
