const { google } = require('googleapis');
const { createClient } = require('@supabase/supabase-js');
const fs = require('fs');
const path = require('path');

// Manual .env loader for Supabase stuff
const envFile = fs.readFileSync(path.join(__dirname, '../.env'), 'utf8');
const env = {};
envFile.split('\n').forEach(line => {
  const idx = line.indexOf('=');
  if (idx > -1) {
    const k = line.substring(0, idx).trim();
    const v = line.substring(idx + 1).trim().replace(/^"|"$/g, '');
    env[k] = v;
  }
});

const s = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY);

// HARDCODED NEW KEY that we know works
const rawKey = "-----BEGIN PRIVATE KEY-----\\ PWp8PfLK1v9TAEhdFFBzPdni9GangPO5PNLASnisfQKBgQDi3CAk4xtXTVPWmYLj\nof7EQIXR6Fiy3hT9xzZSfOFQqdJEt7Y4OvvaHrDq1tVcv5MI/LCst3Mnc/b0njYr\ WVebpwmjKxg7eUVUNnXu4r1n5KCAOMM2p6n2Y5KTh7aBvIEbffO++9NeHduEN5zm\ Rvpw2JNo/cb5NNry3w4b5+iy+wKBgQDeID0GO9aUGZbi3N/wJPf32r5k40n8KmLL\ 4HJtYanaylS6JX6S40oohShKJcyqjtTjLL1L2+vAJyg/5rLnxmG5iHOjtwNJItxR\ \nrpQnResbwH+FQDddgIFwnV/K\n-----END PRIVATE KEY-----\n";
const privateKey = rawKey.split('\\n').join('\n').replace(/ /g, '');
const clientEmail = "survey-drive-bot-311@ruangsinggahid-3afb2.iam.gserviceaccount.com";

const auth = new google.auth.JWT({
  email: clientEmail,
  key: privateKey,
  scopes: ['https://www.googleapis.com/auth/drive.file', 'https://www.googleapis.com/auth/drive']
});
const drive = google.drive({ version: 'v3', auth });

async function createFolder(kostName, orderId) {
    const folderName = `Survey - ${kostName} - ${orderId.substring(0,8).toUpperCase()}`;
    const ROOT_FOLDER_ID = '1KS-uAMJZg7deddNCB4XxRrPpXsQjq1tk';
    
    console.log(`Creating folder for ${kostName}...`);
    const response = await drive.files.create({
      requestBody: {
        name: folderName,
        mimeType: 'application/vnd.google-apps.folder',
        parents: [ROOT_FOLDER_ID],
      },
      fields: 'id, webViewLink',
    });
    
    const fileId = response.data.id;
    await drive.permissions.create({
      fileId: fileId,
      requestBody: { role: 'reader', type: 'anyone' },
    });
    
    return response.data.webViewLink;
}

async function run() {
  const surveys = [
    { id: '65296651-5106-4f96-b04d-4a04090afb18', kost: 'jxk' },
    { id: '663c9efa-dee6-4346-adc5-7c5368c840ec', kost: 'smdklm' },
    { id: '16ea5cc3-4c4d-4d90-8e79-b5e23a05d92e', kost: 'kost apalah' }
  ];

  for (const srv of surveys) {
    try {
      const link = await createFolder(srv.kost, srv.id);
      await s.from('survey_requests')
        .update({ result_drive_link: link })
        .eq('transaction_id', srv.id);
      console.log(`Updated ${srv.kost}: ${link}`);
    } catch (e) {
      console.error(`Failed ${srv.kost}:`, e.message);
    }
  }
}

run();
