...
async function run() {
  const trxId = '05f00c66-bc7c-4edf-937d-33bea223a126';
  console.log("Checking transaction:", trxId);
  const { data, error } = await supabase.from('transactions').select('*').eq('id', trxId).maybeSingle();
  if (error) console.error(error);
  else console.log("Transaction:", data);
}
