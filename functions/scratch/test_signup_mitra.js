...
async function testSignupMitra() {
    const email = `testmitra_${Date.now()}@example.com`;
    const password = "TestPassword123!";
    console.log(`Testing Mitra signup for: ${email}`);
    
    // 1. Sign up the user as 'owner'
    const { data: signUpData, error: signUpError } = await supabase.auth.admin.createUser({
        email,
        password,
        email_confirm: false,
        user_metadata: {
            full_name: "Test Mitra",
            name: "Test Mitra",
            phone: "+628123456789",
            role: "owner"
        }
    ...
    if (confirmError) {
        ...
        // Check if they are in public.users and public.mitra
        const { data: user } = await supabase.from('users').select('*').eq('id', userId).maybeSingle();
        const { data: mitra } = await supabase.from('mitra').select('*').eq('user_id', userId).maybeSingle();
        
        console.log("Public user profile:", user ? "Created!" : "Not created!");
        console.log("Mitra profile:", mitra ? "Created!" : "Not created!");
    }
