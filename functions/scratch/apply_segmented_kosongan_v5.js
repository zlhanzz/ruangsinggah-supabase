...
if (foundTempIdx !== -1) {
    ...
    const newTempBlock = [
        ...
        `${subInd}return (`,
        `${subSubInd}<div className="col-span-2 flex bg-gray-100 p-1 rounded-xl gap-1 mb-2 border border-gray-200/80">`,
        `${subSubSubInd}<button`,
        ...
        `${indentation}})()}`,
        ``,
        `${indentation}{['Kasur', 'Lemari', 'Meja Belajar', 'AC', 'Kipas Angin', 'Water Heater', 'Jendela Luar', 'Kamar Mandi Dalam', 'Dapur Dalam'].map(fac => {`,
        `${subInd}const isChecked = temporaryRoom.roomFacilities?.includes(fac);`,
        `${subInd}const isPerabot = ['Kasur', 'Lemari', 'Meja Belajar', 'AC', 'Kipas Angin', 'Water Heater'].includes(fac);`,
        `${subInd}const isKosongan = temporaryRoom.roomFacilities?.includes('Kosongan (Tanpa Perabot)');`,
        `${subInd}const isDisabled = isPerabot && isKosongan;`,
        `${subInd}return (`,
        `${subSubInd}<label key={fac} className={\`flex items-center gap-2.5 cursor-pointer transition-all \${isDisabled ? 'opacity-40 pointer-events-none' : ''}\`}>`,
        `${subSubInd}    <input `,
        `${subSubInd}        type="checkbox"`,
        `${subSubInd}        checked={isChecked && !isDisabled}`,
        `${subSubInd}        disabled={isDisabled}`,
        `${subSubInd}        onChange={() => {`,
        `${subSubInd}            const current = temporaryRoom.roomFacilities || [];`,
        `${subSubInd}            const updated = current.includes(fac)`,
        `${subSubInd}                ? current.filter((f: string) => f !== fac)`,
        `${subSubInd}                : [...current, fac];`,
        `${subSubInd}            setTemporaryRoom({ ...temporaryRoom, roomFacilities: updated });`,
        `${subSubInd}        }}`,
        `${subSubInd}        className="rounded border-[#e0c0af] text-[#ff7a00] focus:ring-[#ff7a00] w-5 h-5"`,
        `${subSubInd}    />`,
        `${subSubInd}    <span className="text-xs text-gray-700 uppercase tracking-wider font-semibold">{fac}</span>`,
        `${subSubInd}</label>`,
        `${subInd});`,
        `${indentation}})}`
    ];
...
if (foundRtIdx !== -1) {
    ...
    const newRtBlock = [
        ...
        ``,
        `${indentation}{['Kasur', 'Lemari', 'Meja Belajar', 'AC', 'Kipas Angin', 'Water Heater', 'Jendela Luar', 'Kamar Mandi Dalam', 'Dapur Dalam'].map(fac => {`,
        `${subInd}const isChecked = rt.roomFacilities?.includes(fac);`,
        `${subInd}const isPerabot = ['Kasur', 'Lemari', 'Meja Belajar', 'AC', 'Kipas Angin', 'Water Heater'].includes(fac);`,
        `${subInd}const isKosongan = rt.roomFacilities?.includes('Kosongan (Tanpa Perabot)');`,
        `${subInd}const isDisabled = isPerabot && isKosongan;`,
        `${subInd}return (`,
        `${subSubInd}<label key={fac} className={\`flex items-center gap-2.5 cursor-pointer transition-all \${isDisabled ? 'opacity-40 pointer-events-none' : ''}\`}>`,
        `${subSubInd}    <input `,
        ...
        `${subSubInd}        disabled={isDisabled}`,
        `${subSubInd}        onChange={() => {`,
        `${subSubInd}            const current = rt.roomFacilities || [];`,
        `${subSubInd}            const updated = current.includes(fac)`,
        `${subSubInd}                ? current.filter((f: string) => f !== fac)`,
        `${subSubInd}                : [...current, fac];`,
        `${subSubInd}            const updatedRoomTypes = [...kmListingForm.roomTypes];`,
        `${subSubInd}            updatedRoomTypes[activeRoomIdx] = { ...rt, roomFacilities: updated };`,
        `${subSubInd}            setKmListingForm({ ...kmListingForm, roomTypes: updatedRoomTypes });`,
        `${subSubInd}        }}`,
        `${subSubInd}        className="rounded border-[#e0c0af] text-[#ff7a00] focus:ring-[#ff7a00] w-5 h-5"`,
        `${subSubInd}    />`,
        `${subSubInd}    <span className="text-xs text-gray-700 uppercase tracking-wider font-semibold">{fac}</span>`,
