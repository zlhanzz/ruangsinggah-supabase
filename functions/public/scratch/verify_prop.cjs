...
async function check() {
  const { data, error } = await supabase.from('properties').select('id').eq('id', '67f062a8-b5a5-4adb-bd40-928e6e8d9ee6').maybeSingle();
  if (error) console.error(error);
  else console.log(data ? "Property exists!" : "Property MISSING!");
}
check();
