...
async function check() {
  console.log("Looking for kost_booking transactions...");
  const { data, error } = await supabase.from('transactions').select('user_id, product_id, product_type').eq('product_type', 'kost_booking').limit(1);
