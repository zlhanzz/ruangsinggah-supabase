import { config } from \" "https://deno.land/x/dotenv/mod.ts\; const GEMINI_API_KEY = \xxxx\; (async () = await analyzeWithAI('Cek pengajuan sewa', { properties: [{ title: 'Kost Madani' }], stats: { unreadMessages: 15, pendingBookings: 1, pendingSurveys: 0 }, userName: 'Mitra', pendingBookingsList: [{ id: 'b297...', amount: 400000, user: { name: 'zhull' }, metadata: { kostName: 'Kost Madani' } }] }).then(console.log).catch(console.error); })();  
...
async function analyzeWithAI(text: string, context: { properties: any[], stats: any, userName: string, pendingBookingsList: any[] }) {
  ...
  const prompt = `
    ...
    ${context.pendingBookingsList.length === 0 ? "Tidak ada" : context.pendingBookingsList.map((tx: any) => {
       ...
       return `[ID Internal: ${tx.id}]
- Penyewa: ${tName}
- Nama Kost: ${meta.kostName || "Kost"}
- Jenis Kamar: ${meta.item || "-"}
- Jumlah Penghuni: ${meta.bookingDetails?.numberOfGuests || meta.guestCount || 1}
- Tanggal Masuk: ${meta.bookingDetails?.checkInDate || meta.checkInDate || "-"}
...
`;
    ...
    - Jika ada minimal 1 pengajuan, set intent menjadi "ACCEPT_BOOKING" atau "REJECT_BOOKING", dan set "transaction_id" dengan [ID Internal] dari daftar tersebut (ambil yang paling pertama jika user tidak menyebut spesifik).
    ...
    - Tampilkan seluruh data dari "DAFTAR PENGAJUAN SEWA MENUNGGU" di atas dengan rapi.
    - SEMBUNYIKAN/JANGAN TAMPILKAN [ID Internal] panjang (UUID) di teks balasan Anda karena itu hanya untuk sistem Anda sendiri.
    - Akhiri dengan pertanyaan apakah pengajuan tersebut mau diterima atau ditolak.
    ...
    - JANGAN BERIKAN TEMPLATE SAPAAN jika user sedangk menanyakan pengajuan sewa atau mencoba membalas "Terima".
